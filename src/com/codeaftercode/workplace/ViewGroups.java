...
/**
 * 
 * @author http://blog.csdn.net/sunyiz/article/details/6068655 添加组件的方法是： public
 *         void addTab(String title, Icon icon, Component component, String tip,
 *         boolean closable) 移除组件的方法是： public void removeTab(int index) 有个问题：
 *         父类的tabLayoutPolicy参数不能定制，设置为JTabbedPane.SCROLL_TAB_LAYOUT不起作用。 public
...
public class ViewGroups extends JTabbedPane {
	 ...
	 * 构造方法
	 * 
	 * @param window 该参数由调用者传入 关闭选项卡时修改window.ViewsModular下相应CheckBoxMenuItem的值
	 */
	public ViewGroups(Window window) {
		super();
		this.window = window;
		initialize();
	}
	 ...
	 */
	public ViewGroups(Window window, boolean arg0) {
		super();
		this.window = window;
		showClose = arg0;
		initialize();
	...
	public void addTab(String title, Icon icon, Component component, String tip, boolean closable,
		...
		// 检查是否存在重复项
		if (arrayList.contains(myJCheckBoxMenuItem)) {
			// 存在重复项:不用重复添加

		} else {
			// 不存在重复项:
			addTab(title, icon, component, tip);
			arrayList.add(myJCheckBoxMenuItem);
	...
	public void removeTab(int index) {
		...
		// 根据索引取出对应的菜单项
		JCheckBoxMenuItem jCheckBoxMenuItem = arrayList.get(index);
	...
	public ArrayList<MyJCheckBoxMenuItem> getArrayList() {
		return arrayList;
	}

	public void setArrayList(ArrayList<MyJCheckBoxMenuItem> arrayList) {
	 ...
	 * @author Sun
	 */
	class ViewGroupsUI extends BasicTabbedPaneUI {
		...
		private void initialize() {
			...
			/*
			 * //注册鼠标移动事件--不知何用 addMouseMotionListener(new MouseAdapter() {
			 * public void mouseMoved(MouseEvent e) { //获取鼠标所在的选项卡索引 nowIndex =
			 * -1; int tabCounter = getTabCount(); //如果选项卡数<2,则返回.(不加这句会报错)
			 * if(tabCounter < 2) { return; } for (int i = 0; i < tabCounter;
			 * i++) { //如果没有上述判断,在第1次打开文件且工作区为空时,下面的if条件会报错 //现在还是会报错
			 * //"AWT-EventQueue-0" java.lang.ArrayIndexOutOfBoundsException: 0
			 * if (closeRects[i].contains(e.getPoint()) && closable.get(i)) {
			 * nowIndex = i; break; } } if (oldIndex != nowIndex) { if (nowIndex
			 * != -1) { repaint(closeRects[nowIndex]);// 控制重绘区域 } else { if
			 * (oldIndex < tabCounter) { repaint(closeRects[oldIndex]);// 控制重绘区域
			 * } } oldIndex = nowIndex; } } });
			 */
		}
